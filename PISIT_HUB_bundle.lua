--[[
	...
	(Red / Blue / Green / Purple / Light) via Library:SetTheme("Blue") or
...
-- เพิ่มใหม่: สลับธีมแบบ live ทุก element ที่ผูก OnChanged ไว้จะอัปเดตสีทันที
...
function Button.new(parent, config)
	...
	-- เพิ่มใหม่: อัปเดตสีทันทีเมื่อสลับธีม
...
function ColorPicker.new(parent, config)
	...
	-- เพิ่มใหม่: อัปเดตสีทันทีเมื่อสลับธีม (แถบ R/G/B ยังคงสีของมันเอง ปรับแค่พื้นหลัง/กรอบ)
	Theme.OnChanged:Connect(function()
		self.Instance.BackgroundColor3 = Theme.Get("ElementBackground")
		stroke.Color = Theme.Get("Border")
		titleLbl.TextColor3 = Theme.Get("Text")
	...
	-- เพิ่มใหม่: รองรับ Config Save/Load ผ่าน Flag
	if config.Flag then
		Modules.Config.Register(config.Flag, {
...
-- เพิ่มใหม่: รูปภาพ (ใช้ asset id หรือ url ก็ได้ถ้าเปิดใช้ insertservice/decal)
...
-- เพิ่มใหม่: ดรอปดาวน์สำหรับสลับธีมของทั้ง UI แบบ live
...
function Window.new(config)
	...
	-- เพิ่มใหม่: อัปเดตสีทันทีเมื่อสลับธีมทั้งหน้าต่างหลัก
	Theme.OnChanged:Connect(function()
...
-- เพิ่มใหม่ (v2.2.0): สลับธีมทั้งไลบรารีแบบ live + ดูรายชื่อธีมที่มี
